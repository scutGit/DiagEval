...
######### Copy project ###########
WORKDIR /workspace/DiagEval
COPY . /workspace/DiagEval

######### Virtualenv + Python Dependencies ###########
# Create venv (with system site packages per README), install deps into venv via pip
ARG ENABLE_ULTRA=0
RUN set -eux; \
    python3 -m venv /workspace/DiagEval/venv --system-site-packages; \
    . /workspace/DiagEval/venv/bin/activate; \
...
ENV VIRTUAL_ENV=/workspace/DiagEval/venv
