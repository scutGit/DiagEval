...
If the target application requires login or registration, use the website's own built-in credentials first (e.g., pre-filled values in the login form, or credentials specified in the task instruction). Only if the website has no built-in credentials, fall back to: Email: test_user@example.com, Password: 123456. Do NOT register a new account or go through the sign-up flow.
"""
